...
class EpaApp {
  ...
  async renderFolder() {
    ...
    this.$app.innerHTML = `
              ...
              ${files.map((f, i) => {
                ...
                return `
                  <div class="file-item animate-slide-up delay-${i % 5}"
                       onclick="app.showFileActions('${f.id}','${this.escAttr(f.name)}','${f.type}',${f.size})">
      ...
      <!-- File action sheet -->
      <div id="modal-file-actions" class="modal hidden">
        <div class="modal-overlay" onclick="app.closeModal('modal-file-actions')"></div>
        <div class="modal-card faction-card">
          <div class="modal-handle"></div>
          <div class="faction-header">
            <div id="faction-icon" class="faction-file-icon"></div>
            <div class="faction-file-info">
              <div id="faction-name" class="faction-file-name"></div>
              <div id="faction-meta" class="faction-file-meta"></div>
            </div>
          </div>
          <div class="faction-list">
            <button class="faction-btn" onclick="app.openWithApp()">
              <span class="faction-btn-ico">📲</span>
              <div class="faction-btn-text">
                <span class="faction-btn-label">Ouvrir avec une application</span>
                <span class="faction-btn-sub">Word, Acrobat, Galerie, VLC…</span>
              </div>
              <span class="faction-chevron">›</span>
            </button>
            <button class="faction-btn" onclick="app.openInBrowser()">
              <span class="faction-btn-ico">🌐</span>
              <div class="faction-btn-text">
                <span class="faction-btn-label">Ouvrir dans le navigateur</span>
                <span class="faction-btn-sub">Aperçu rapide intégré</span>
              </div>
              <span class="faction-chevron">›</span>
            </button>
            <button class="faction-btn" onclick="app.downloadCurrent()">
              <span class="faction-btn-ico">⬇️</span>
              <div class="faction-btn-text">
                <span class="faction-btn-label">Télécharger</span>
                <span class="faction-btn-sub">Enregistrer sur l'appareil</span>
              </div>
              <span class="faction-chevron">›</span>
            </button>
            <button class="faction-btn faction-btn-danger" onclick="app.deleteCurrent()">
              <span class="faction-btn-ico">🗑️</span>
              <div class="faction-btn-text">
                <span class="faction-btn-label">Supprimer</span>
                <span class="faction-btn-sub">Retirer définitivement ce fichier</span>
              </div>
              <span class="faction-chevron">›</span>
            </button>
          </div>
          <button class="faction-cancel" onclick="app.closeModal('modal-file-actions')">Annuler</button>
        </div>
      </div>`;
  }
  ...
  // ── Feuille d'actions fichier ──
  showFileActions(id, name, type, size) {
    this._activeFile = { id, name, type, size };
    const ft = getFileType(type);
    document.getElementById('faction-icon').textContent = ft.icon;
    document.getElementById('faction-name').textContent = name;
    document.getElementById('faction-meta').textContent = `${ft.label} · ${fmtSize(size)}`;
    document.getElementById('modal-file-actions').classList.remove('hidden');
  }

  async openWithApp() {
    this.closeModal('modal-file-actions');
    const { id, name, type } = this._activeFile;
    const file = await this.db.getFile(id);
    if (!file) return;

    const blob = new Blob([file.data], { type });

    // iOS uniquement : la feuille de partage iOS inclut "Ouvrir dans…"
    // (Word, Acrobat, VLC…) en haut — c'est le mécanisme natif iOS
    ...
    // Android & PC : téléchargement du fichier avec son nom et extension
    // → Android : notification "Ouvrir" → sélecteur natif "Ouvrir avec [app]"
    // → PC      : fichier dans Téléchargements → ouvrir avec l'app choisie
    ...
    // Guide contextuel selon la plateforme
    const isAndroid = /Android/i.test(navigator.userAgent);
    this.toast(
      isAndroid
        ? 'Appuyez sur "Ouvrir" dans la notification pour choisir votre application'
        : 'Fichier prêt dans vos téléchargements — ouvrez-le avec l\'application souhaitée',
      'info'
    );
  }
  ...
  async openInBrowser() {
    this.closeModal('modal-file-actions');
    const file = await this.db.getFile(this._activeFile.id);
    if (!file) return;
  ...
  async downloadCurrent() {
    this.closeModal('modal-file-actions');
    await this.downloadFile(this._activeFile.id);
  }

  async deleteCurrent() {
    this.closeModal('modal-file-actions');
    await this.deleteFile(this._activeFile.id, this._activeFile.name);
  }
