...
class EpaApp {
  ...
  navigate(view, params = {}) {
    ...
    this.render();
    window.scrollTo({ top: 0, behavior: 'instant' });
    this.track('page_view', { page: view, ...params });
  }
  ...
  async createFolder(dayKey) {
    ...
    this.track('folder_created', { week: this.state.weekId, day: this.state.dayId });
  ...
  async importFiles(event, folderId) {
    ...
    if (ok) {
      this.toast(`${ok} fichier${ok > 1 ? 's' : ''} importé${ok > 1 ? 's' : ''}`, 'success');
      this.track('files_imported', { count: ok, folder: this.state.folderId });
    }
    event.target.value = '';
  ...
  async openInBrowser(id) {
    // iOS Safari bloque window.open() après une opération async
    // Solution : ouvrir l'onglet vide immédiatement (pendant le tap)
    // puis y injecter le fichier après lecture IndexedDB
    ...
    try {
      ...
      if (!file || !file.data) {
        newTab?.close();
        this.toast('Fichier introuvable', 'error');
        return;
      }
      ...
      if (newTab) {
        ...
        // Fallback : afficher dans la même page si l'onglet est bloqué
      ...
      newTab?.close();
      this.toast('Erreur lors de l\'ouverture', 'error');
    }
  }
