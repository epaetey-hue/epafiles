...
class EpaApp {
  ...
  async openWithApp() {
    ...
    const shareF = new File([blob], name, { type });

    // Tentative Web Share API (mobile Android/iOS, macOS Safari)
    if (navigator.canShare?.({ files: [shareF] })) {
      try {
        await navigator.share({ files: [shareF], title: name });
        return;
      } catch (e) {
    ...
    // Fallback PC : téléchargement → l'OS propose d'ouvrir avec une app
    const url = URL.createObjectURL(blob);
    Object.assign(document.createElement('a'), { href: url, download: name }).click();
    setTimeout(() => URL.revokeObjectURL(url), 5000);
    this.toast('Ouvrez le fichier téléchargé avec l\'app de votre choix', 'info');
  }
